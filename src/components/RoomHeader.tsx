...
const STATUS_CONFIG: Record<
  VoteStatus,
  { labelKey: string; fallback: string; color: string; bg: string; dot: string }
> = {
  open: {
...
export default function RoomHeader() {
  ...
  const status = roomInfo
    ? STATUS_CONFIG[roomInfo.status] || STATUS_CONFIG.closed
    ...
    <header className="sticky top-0 z-10 border-b border-slate-200 bg-white shadow-sm">
      <div className="mx-auto max-w-lg px-4 py-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <span
            className={`inline-flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${status.bg} ${status.color}`}
          >
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px] text-slate-500">
          {roomInfo.timeMode === "duration" ? (
            <span className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-1">
              ໄລຍະເວລາ: {roomInfo.durationMinutes} ນາທີ
            </span>
          ) : (
            <>
