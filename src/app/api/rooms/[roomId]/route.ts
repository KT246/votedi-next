...
import { autoCloseExpiredRoom, getRoomDeadline, getVoteRoomKeys } from '@/lib/roomLifecycle';
...
function normalizeCandidates(value: unknown): Candidate[] {
    ...
    return value.map((item, index) => {
        ...
        return {
            id: normalizeString(candidate.id ?? candidate._id) || `candidate-${index + 1}`,
...
async function findRoomByKey(db: { collection: (name: string) => { findOne: (query: Record<string, unknown>) => Promise<RoomDocument | null> } }, roomKey: string) {
    ...
    if (ObjectId.isValid(roomKey) && String(new ObjectId(roomKey)) === roomKey) {
        const byId = await db.collection('rooms').findOne({ _id: new ObjectId(roomKey) });
        if (byId) return byId;
    }
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ roomId: string }> }) {
    ...
    const requestedStatus = body?.status !== undefined ? normalizeStatus(body.status) : undefined;
    const update: Record<string, unknown> = {
        updatedAt: new Date(),
    };

    if (body?.roomName !== undefined) update.roomName = normalizeString(body.roomName);
    if (body?.description !== undefined) update.description = normalizeString(body.description);
    if (body?.startTime !== undefined) update.startTime = body.startTime ? new Date(String(body.startTime)) : null;
    if (body?.endTime !== undefined) update.endTime = body.endTime ? new Date(String(body.endTime)) : null;
    if (body?.timeMode !== undefined) update.timeMode = body.timeMode === 'duration' ? 'duration' : 'range';
    if (body?.durationMinutes !== undefined) update.durationMinutes = Number.isFinite(Number(body.durationMinutes)) ? Number(body.durationMinutes) : undefined;
    if (body?.voteType !== undefined) update.voteType = normalizeVoteType(body.voteType);
    if (body?.maxSelection !== undefined) update.maxSelection = Number.isFinite(Number(body.maxSelection)) && Number(body.maxSelection) > 0 ? Number(body.maxSelection) : 1;
    if (body?.allowResultView !== undefined) update.allowResultView = Boolean(body.allowResultView);
    if (body?.candidates !== undefined) update.candidates = normalizeCandidates(body.candidates);
    if (body?.allowedUsers !== undefined) {
        update.allowedUsers = Array.isArray(body.allowedUsers) ? body.allowedUsers.map((value: unknown) => String(value)) : [];
    }
    ...
    const isClosedToResetStatus = currentRoom.status === 'closed' && (requestedStatus === 'open' || requestedStatus === 'draft');
    ...
    if (requestedStatus === 'open') {
        update.status = 'open';
        const wantsRange = body?.timeMode === 'range' && update.startTime instanceof Date && update.endTime instanceof Date;
        const effectiveTimeMode = (update.timeMode as 'duration' | 'range' | undefined) || currentRoom.timeMode || 'duration';
        if (wantsRange) {
            update.timeMode = 'range';
        } else if (effectiveTimeMode === 'duration' || currentRoom.status === 'closed') {
            const durationMinutes = Number.isFinite(Number(update.durationMinutes)) && Number(update.durationMinutes) > 0
                ? Number(update.durationMinutes)
                : Number.isFinite(Number(currentRoom.durationMinutes)) && Number(currentRoom.durationMinutes) > 0
                    ? Number(currentRoom.durationMinutes)
                    : 60;
            const startTimeValue = update.startTime instanceof Date
                ? update.startTime
                : currentRoom.startTime instanceof Date
                    ? currentRoom.startTime
                    : new Date();
            const startTimeMs = startTimeValue.getTime();
            const resolvedStartTime = Number.isNaN(startTimeMs) ? new Date() : startTimeValue;
            update.startTime = resolvedStartTime;
            update.endTime = new Date(resolvedStartTime.getTime() + durationMinutes * 60 * 1000);
    ...
    const filter: Record<string, unknown> = currentRoom._id ? { _id: currentRoom._id } : { roomCode: currentRoom.roomCode };
    const result = await auth.db.collection('rooms').updateOne(filter, { $set: update });
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ roomId: string }> }) {
    ...
    const result = await auth.db.collection('rooms').deleteOne({ _id: currentRoom._id });
