...
export default function AdminUsersPage() {
    ...
    const [resetPasswordToStudentId, setResetPasswordToStudentId] = useState(false);
    ...
    const fetchUsers = useCallback(async () => {
        ...
        try {
            ...
            setError(Array.isArray(message) ? message.join(', ') : message || typedErr?.message || 'ບໍ່ສາມາດໂຫຼດຜູ້ໃຊ້ໄດ້');
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const handleImportFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const parsed = await new Promise<{ data: Record<string, unknown>[] }>((resolve, reject) => {
                Papa.parse<Record<string, unknown>>(file, {
                    header: true,
                    skipEmptyLines: true,
                    complete: (result) => resolve({ data: (result.data || []) as Record<string, unknown>[] }),
            ...
            const created = Array.isArray(res.data?.created) ? res.data.created.map(normalizeUser) : [];
            ...
            setImportMessage(`ນຳເຂົ້າສຳເລັດ ${createdCount} ລາຍການ${skippedCount > 0 ? `, ຂ້າມ ${skippedCount} ລາຍການ` : ''}`);
        } catch (err: unknown) {
            const typedErr = err as { response?: { data?: { message?: string | string[] } }; message?: string };
            const message = typedErr?.response?.data?.message;
            setImportError(Array.isArray(message) ? message.join(', ') : message || typedErr?.message || 'ນຳເຂົ້າ CSV ບໍ່ສຳເລັດ');
        } finally {
            setImporting(false);
        }
    };
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        if (!form.username.trim() || !form.fullName.trim() || !form.studentId.trim()) {
        ...
        try {
            ...
            const typedErr = err as { response?: { data?: { message?: string | string[] } }; message?: string };
            const message = typedErr?.response?.data?.message;
            setError(Array.isArray(message) ? message.join(', ') : message || typedErr?.message || 'ບັນທຶກຜູ້ໃຊ້ບໍ່ສຳເລັດ');
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleDelete = async (user: ManagedUser) => {
        const confirmed = window.confirm(`ຕ້ອງການລົບຜູ້ໃຊ້ "${user.username}" ຫຼືບໍ?`);
        if (!confirmed) return;
        ...
        try {
            ...
            const typedErr = err as { response?: { data?: { message?: string | string[] } }; message?: string };
            const message = typedErr?.response?.data?.message;
            setError(Array.isArray(message) ? message.join(', ') : message || typedErr?.message || 'ລົບຜູ້ໃຊ້ບໍ່ສຳເລັດ');
        } finally {
            setDeletingId('');
        }
    };
    ...
    const stats = useMemo(
        () => ({
            total: users.length,
            needPasswordChange: users.filter((user) => user.mustChangePassword).length,
        }),
        [users],
    );
            ...
            <div className="min-h-screen bg-slate-50 p-6">
                <div className="mx-auto max-w-7xl">
                    <div className="mb-8 flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <h1 className="text-2xl font-bold text-slate-900">ຈັດການຜູ້ໃຊ້</h1>
                            <p className="text-slate-500">
                                ສ້າງ, ແກ້ໄຂ ແລະລົບບັນຊີຜູ້ໃຊ້. ລະຫັດຜ່ານເລີ່ມຕົ້ນແມ່ນມາຈາກ student ID
                            </p>
                        </div>
                        <button
                            onClick={resetForm}
                    ...
                    <div className="mb-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                        <div className="flex flex-wrap items-center justify-between gap-3">
                            <div>
                                <p className="text-sm font-semibold text-slate-900">ນຳເຂົ້າຜູ້ໃຊ້ຈາກ CSV</p>
                                <p className="text-xs text-slate-500">ຄໍລຳທີ່ຮອງຮັບ: username, fullName, studentId</p>
                            </div>
                        ...
                        {importing ? <p className="mt-3 text-xs text-slate-500">ກຳລັງນຳເຂົ້າ CSV...</p> : null}
                    </div>
                    ...
                    <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                            ...
                            <p className="mt-2 text-3xl font-bold text-slate-900">{stats.needPasswordChange}</p>
                        </div>
                    </div>
                                ...
                                <div className="overflow-x-auto">
                                    <table className="min-w-full divide-y divide-slate-200">
                                        <thead className="bg-slate-50">
                                            <tr>
                                                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Username</th>
                                        ...
                                        <tbody className="divide-y divide-slate-200">
                                            {filteredUsers.map((user) => (
                                                <tr key={user.id} className="hover:bg-slate-50/70">
                                                    <td className="px-4 py-4 font-mono text-sm text-slate-900">{user.username}</td>
                                                    <td className="px-4 py-4 text-sm text-slate-700">{user.fullName}</td>
                                                    <td className="px-4 py-4 font-mono text-sm text-slate-600">{user.studentId}</td>
                                                    <td className="px-4 py-4 text-sm">
                                                        ...
                                                        >
                                                            {user.mustChangePassword ? 'ຕ້ອງປ່ຽນລະຫັດ' : 'ພ້ອມໃຊ້ງານ'}
                                                        </span>
                                                    </td>
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                            <div className="mb-4 flex items-start justify-between gap-3">
                                <div>
                                    <h2 className="text-lg font-bold text-slate-900">
                                        {editingUser ? 'ແກ້ໄຂຜູ້ໃຊ້' : 'ສ້າງຜູ້ໃຊ້ໃໝ່'}
                                    </h2>
                                    <p className="mt-1 text-sm text-slate-500">
                                        Username ແມ່ນໄວ້ເຂົ້າລະບົບ, student ID ແມ່ນລະຫັດຜ່ານເລີ່ມຕົ້ນ
                                    </p>
                                </div>
                                {editingUser ? (
                                    <button
                                        onClick={resetForm}
                                        className="inline-flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-50"
                                    >
                                        <X className="h-4 w-4" />
                                        ຍົກເລີກ
                                    </button>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                {editingUser ? (
                                    <label className="flex items-start gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-600">
                                        <input
                                            ...
                                            onChange={(event) => setResetPasswordToStudentId(event.target.checked)}
                                    ...
                                    <div className="rounded-xl border border-indigo-100 bg-indigo-50 px-3 py-2 text-xs text-indigo-700">
                                        ລະຫັດຜ່ານເລີ່ມຕົ້ນຈະຖືກ hash ຈາກ student ID ແລະບັນຊີຈະຖືກບັງຄັບປ່ຽນລະຫັດຫຼັງຈາກເຂົ້າລະບົບຄັ້ງທຳອິດ
                                    </div>
                                )}
                                ...
                                <div className="flex gap-2">
                                    ...
                                    >
                                        {saving ? 'ກຳລັງບັນທຶກ...' : editingUser ? 'ບັນທຶກການແກ້ໄຂ' : 'ສ້າງຜູ້ໃຊ້'}
                                    </button>
                                </div>
                            </form>
                        </div>
